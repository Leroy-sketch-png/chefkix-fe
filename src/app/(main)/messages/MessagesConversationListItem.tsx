...
function ConversationItem({
	...
	const hasLastMessage = Boolean(conversation.lastMessage?.createdDate)
	const previewText = conversation.lastMessage?.message || t('noMessagesYet')
	const previewDate = conversation.lastMessage?.createdDate
	const isGroupConversation = conversation.type === 'GROUP'
	const participantCount = conversation.participants.length
	const contextBadge = isGroupConversation
		...
		>
			<div
				className={cn(
					...
					hasLastMessage ? 'size-11' : 'size-10',
				)}
			>
				<Image
					src={avatar}
