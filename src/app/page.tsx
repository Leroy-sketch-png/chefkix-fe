...
export default function Home() {
	...
	useEffect(() => {
		...
		if (
			isGameStarted &&
			gamifiedRecipe &&
			currentStepIndex < gamifiedRecipe.steps.length
		) {
			...
			<div>
				...
				<div className='mb-4'>
					...
					{gamifiedRecipe.badges.map((badge, index) => (
						<span
							key={index}
							className='inline-block bg-green-200 text-green-800 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded-full'
						>
							{badge}
						</span>
					))}
				...
				<div className='border-b py-2 last:border-b-0'>
					...
					<ul className='ml-4 mt-2 text-sm text-gray-700'>
						...
						<li>
							...
							{currentStep.action || 'N/A'}
						</li>
						<li>
							<span className='font-medium'>Ingredients:</span>{' '}
							{currentStep.ingredients?.join(', ') || 'N/A'}
						</li>
					</ul>
				</div>
