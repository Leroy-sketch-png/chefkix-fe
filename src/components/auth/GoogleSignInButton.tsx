'use client'

import React from 'react'
import { SocialLoginButton } from '@/components/ui/social-login-button'
import { useGoogleLogin } from '@react-oauth/google'

interface GoogleSignInButtonProps {
	onSuccess: (code: string) => void
	onFailure: (error: any) => void
	text?: string
}

/**
 * Internal component that uses the Google OAuth hook.
 * Only rendered when Google OAuth is configured.
 */
const GoogleSignInButtonWithOAuth: React.FC<GoogleSignInButtonProps> = ({
	onSuccess,
	onFailure,
	text = 'Sign in with Google',
}) => {
	const login = useGoogleLogin({
		onSuccess: tokenResponse => {
			if (tokenResponse.code) {
				onSuccess(tokenResponse.code)
			} else {
				onFailure(new Error('No authorization code found in Google response'))
			}
		},
		onError: error => {
			onFailure(error)
		},
		flow: 'auth-code',
	})

	return (
		<SocialLoginButton
			onClick={() => login()}
			icon={
				<svg className='h-5 w-5' viewBox='0 0 24 24'>
					<path
						fill='#4285F4'
						d='M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z'
					/>
					<path
						fill='#34A853'
						d='M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z'
					/>
					<path
						fill='#FBBC05'
						d='M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z'
					/>
					<path
						fill='#EA4335'
						d='M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z'
					/>
				</svg>
			}
			label={text}
		/>
	)
}

/**
 * Disabled placeholder when Google OAuth is not configured.
 */
const GoogleSignInButtonDisabled: React.FC<{
	onFailure: (error: any) => void
	text: string
}> = ({ onFailure, text }) => {
	return (
		<SocialLoginButton
			onClick={() => {
				onFailure(
					new Error(
						'Google Sign-In is not configured. Please contact support.',
					),
				)
			}}
			disabled
			icon={
				<svg className='h-5 w-5 opacity-50' viewBox='0 0 24 24'>
					<path
						fill='#9CA3AF'
						d='M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z'
					/>
					<path
						fill='#9CA3AF'
						d='M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z'
					/>
					<path
						fill='#9CA3AF'
						d='M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z'
					/>
					<path
						fill='#9CA3AF'
						d='M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z'
					/>
				</svg>
			}
			label={`${text} (Coming Soon)`}
		/>
	)
}

/**
 * Google Sign-In button that gracefully handles missing configuration.
 * Shows "Coming Soon" when NEXT_PUBLIC_GOOGLE_CLIENT_ID is not set.
 */
const GoogleSignInButton: React.FC<GoogleSignInButtonProps> = props => {
	const { text = 'Sign in with Google', onFailure } = props
	const isGoogleConfigured = !!process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID

	if (!isGoogleConfigured) {
		return <GoogleSignInButtonDisabled onFailure={onFailure} text={text} />
	}

	return <GoogleSignInButtonWithOAuth {...props} />
}

export default GoogleSignInButton
