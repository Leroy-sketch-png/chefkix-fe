...
async function tryCatch(
	...
	try {
		...
		ghost.log(
			'warn',
			`[RECOVERY INITIATED] Sequence beat failed: ${name}. Reason: ${e.message}. Proceeding to next beat.`,
		)
	}
}
...
export async function runOmniDemo() {
	...
	try {
		...
		await tryCatch(mainGhost, 'Hero Recipe', async () => {
			...
			const firstRecipe =
				'[data-testid="recipe-card-link"], main a[href*="/recipes/"]'
			...
			const checkbox =
				'[data-testid="ingredient-checkbox"], input[type="checkbox"], [role="checkbox"]'
			try {
			...
			// Chat with AI
			const chatInput =
				'[data-testid="ai-chat-input"], [data-testid="room-chat-input"], input[placeholder*="ask" i]'
			try {
				await mainGhost.type(
					chatInput,
					'Can I substitute butter for olive oil?',
				)
				await mainGhost.press(chatInput, 'Enter')
				// Deterministic Sync: Wait for AI message to render!
				await mainGhost.waitForSelector(
					'[data-testid="ai-message"], .message-bot, .ai-response',
					15000,
				)
				await mainGhost.wait(3500) // Paced read for AI response after it appears
			} catch {}

			// Start Cooking
			const startCooking =
				'[data-testid="start-cooking-btn"], button:has-text("Start Cooking"), button:has-text("Cook Recipe")'
			await mainGhost.waitForSelector(startCooking, 10000)
		...
		await tryCatch(mainGhost, 'Co-Cook Multi-Cursor', async () => {
			...
			const roomInput =
				'[data-testid="room-code-input"], input[placeholder*="room" i], input[maxlength="6"]'
			await mainGhost.type(roomInput, 'ABC123')
			await mainGhost.click(
				'[data-testid="join-room-btn"], button:has-text("Join Room"), button:has-text("Join")',
			)

			// Deterministic Sync: Wait for chat input to mount after backend cold start
			const chatInput =
				'[data-testid="co-cook-chat-input"], input[placeholder*="chat" i], input[placeholder*="message" i]'
			await mainGhost.waitForSelector(chatInput, 15000)
			...
			const readyCheck =
				'[data-testid="step-ready-btn"], button:has-text("Ready"), input[type="checkbox"], [role="checkbox"]'
			try {
				// Friend hovers first checkbox
				await friendGhost.hoverMatch(readyCheck, 0, true)
				await friendGhost.clickMatch(readyCheck, 0)
				await friendGhost.wait(1000)

				// Main cursor checks off step 2
				await mainGhost.hoverMatch(readyCheck, 1, true)
				await mainGhost.clickMatch(readyCheck, 1)
				await mainGhost.wait(1500)
			} catch {}
		...
		await tryCatch(mainGhost, 'Taste Compatibility', async () => {
			...
			await mainGhost.waitForSelector('a[href="/profile/taste"]', 10000)
			await mainGhost.click('a[href="/profile/taste"]')

			// Sweep the taste radar with the spotlight
			const radar =
				'[data-testid="taste-radar"] svg, [data-testid="taste-radar"] canvas'
			await mainGhost.waitForSelector(radar, 15000)
			try {
			...
			const tonightsPick =
				'[data-testid="tonights-pick-link"], [data-testid="tonights-pick"] a'
			await mainGhost.waitForSelector(tonightsPick, 15000)
		...
		await tryCatch(mainGhost, 'Creator Heatmap', async () => {
			...
			await mainGhost.waitForSelector(
				'[data-testid="creator-analytics-card"], .analytics-card',
				15000,
			)

			// Spotlight top-level metrics
			try {
				await mainGhost.hover(
					'[data-testid="creator-analytics-card"], .analytics-card',
					true,
				)
				await mainGhost.wait(2000)
			...
			const viewAnalytics =
				'[data-testid="view-step-analytics-btn"], button:has-text("View Step Analytics"), button:has-text("Analytics")'
			await mainGhost.waitForSelector(viewAnalytics, 10000)
			try {
				...
				// Sweep heatmap bars
				const heatmapBars =
					'[data-testid*="heatmap-bar"], [data-testid*="step-bar"], svg rect'
				await mainGhost.waitForSelector(heatmapBars, 15000)
		...
		await tryCatch(mainGhost, 'Year In Cooking', async () => {
			...
			const yicLink =
				'a[href*="/year-in-cooking"], a:has-text("Year in Cooking")'
			try {
				await mainGhost.click('a[href="/dashboard"], a[href="/profile"]')
				await mainGhost.waitForSelector(yicLink, 10000)
				...
				await mainGhost.waitForSelector(nextBtn, 10000)
		...
		await tryCatch(mainGhost, 'Admin Reports', async () => {
			...
			try {
				...
				// Resolve a report
				const resolveBtn =
					'button:has-text("Resolve"), button:has-text("Action")'
				await mainGhost.waitForSelector(resolveBtn, 15000)
		...
		mainGhost.log(
			'error',
			`CRITICAL FAILURE. Halting Ghost Driver. Reason: ${e.message}`,
		)
